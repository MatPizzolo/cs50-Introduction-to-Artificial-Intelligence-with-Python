...
def shortest_path(source, target):
    ...
    while not frontier.empty():
        ...
        explored_path.add(current_person_id)

        for (movie_id, person_id) in neighbors_for_person(current_person_id): 
            if person_id not in explored_path and not frontier.contains_state(person_id):
                if person_id == target:
                    goal_node = Node(state=person_id, parent=current_node, action=movie_id)
